...
exports.postComment = async (req, res) => {
  try {
    ...
    //2. create comment{userReference}
    //3. push comment into video comment array
...
exports.deleteComment = async (req, res) => {
  try {
    ...
    // 2. check if the comment is written by current users
...
exports.likeVideoPost = async (req, res) => {
  try {
    ...
    if (!post) {
      return res.status(404).json({
        msg: 'Cannot find the video with videoPostId.',
      });
    }
...
exports.createVideoPost = async (req, res) => {
  ...
  // 1. get the metadata from client
  const {
    postTitle,
    imageUrl,
    videoUrl,
